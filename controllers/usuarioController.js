...
module.exports = {
  ...
  login: function (req, res) {
    ...
    db.Usuario.findOne({
      where: {
        [op.or]: [{ email: req.body.nombre }, { nombre: req.body.nombre }],
      },
    }).then(function (usuario) {
      if (!usuario) {
        res.redirect("/usuario/login?error=usuario");
      } else if (
        bcrypt.compareSync(req.body.password, usuario.password) == false
      ) {
        res.redirect("/usuario/login?error=password");
      } else {
        req.session.usuarioLogueado = usuario;
        if (req.body.recordar != undefined) {
          res.cookie("usuarioLogueadoId", usuario.id, {
  ...
  registroCheck: function (req, res) {
    ...
    db.Usuario.findOne({
      where: {
        [op.or]: [{ email: req.body.email }, { nombre: req.body.nombre }],
      },
    }).then(function (usuario) {
      if (usuario == null) {
  ...
  borrarComentario: function (req, res) {
    ...
    db.Comentario.findByPk(id).then(function (comentario) {
      res.render("borrarComentario", {
